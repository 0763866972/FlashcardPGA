...
const newPrompt = `            const prompt = \`Nhiệm vụ của bạn là trích xuất và phân tích các cụm TỪ VỰNG hay VÀ các cấu trúc NGỮ PHÁP nổi bật (ví dụ: các thì đang dùng, mệnh đề quan hệ, rút gọn mệnh đề, câu bị động, cấu trúc V-ing/to-V...) từ đoạn văn bản sau.

YÊU CẦU TRÌNH BÀY (BẮT BUỘC, CHỈ DÙNG HTML, KHÔNG DÙNG CODE BLOCK \\\`\\\`\\\`, KHÔNG SỬA LỖI, KHÔNG NHẬN XÉT):

<h3 class="font-bold text-xl text-brand-600 mb-3">1. Phân tích Từ vựng & Ngữ pháp (Đoạn văn gốc)</h3>
<div class="mb-6 text-lg leading-relaxed bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
  (Viết lại TOÀN BỘ đoạn văn bản gốc ở đây. Hãy bôi đậm và tô màu bằng thẻ <span style="color: [mã màu]; font-weight: bold;">...</span> cho CẢ TỪ VỰNG LẪN CẤU TRÚC NGỮ PHÁP. Mỗi cụm dùng một màu KHÁC NHAU ví dụ #ef4444, #f59e0b, #10b981, #3b82f6...)
</div>

<h3 class="font-bold text-xl text-brand-600 mb-3">2. Giải thích chi tiết</h3>
(Liệt kê và giải thích các cụm từ/cấu trúc đã bôi màu ở phần 1. MÀU SẮC PHẢI KHỚP HOÀN TOÀN với màu ở phần 1. Mỗi mục dùng cấu trúc HTML sau:)
<div class="mb-4 text-lg bg-white p-3 rounded-lg border border-slate-100 shadow-sm">
  <span style="color: [MÃ MÀU KHỚP VỚI PHẦN 1]; font-weight: bold;">[Từ vựng / Cấu trúc tiếng Anh]:</span> 
  <span class="text-slate-700">Giải nghĩa tiếng Việt ngắn gọn và giải thích cách dùng ngữ pháp.</span>
</div>
...
console.log("Updated prompt to remove error checking");
