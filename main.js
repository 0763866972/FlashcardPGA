...
Ví dụ CHUẨN trong structures: struct: "[be eligible for] + {noun}", vi: "[đủ điều kiện cho] + {danh từ}", example: "She is [eligible for] {the scholarship}.", example_vi: "Cô ấy [đủ điều kiện cho] {học bổng}."`;
        ...
        if (wantFam) taskInstructions += `\n4. KHAI THÁC TỐI ĐA TỪ CÙNG GỐC (Word Family): Hãy tìm VÀ LIỆT KÊ TOÀN BỘ tất cả các biến thể của từ (danh từ, động từ, tính từ, trạng từ, các từ thêm tiền tố/hậu tố như un-, in-, -ness, -ity, -tion, -ment...). BẮT BUỘC TÌM ÍT NHẤT 4-5 TỪ CÙNG GỐC (nếu từ điển có), TUYỆT ĐỐI KHÔNG ĐƯỢC LƯỜI BIẾNG CHỈ TÌM 1-2 TỪ. Đánh dấu "isSpecial": true nếu từ có dạng đuôi dễ nhầm lẫn (vd: danh từ nhưng đuôi -al, tính từ đuôi -ing/-ed). Đánh dấu "isDifferentMeaning": true nếu từ đó CÓ NGHĨA KHÁC HOÀN TOÀN so với từ gốc (vd: confidence là tự tin, nhưng confidential là tuyệt mật). BẮT BUỘC PHẢI TRẢ VỀ TRONG KEY 'family'.`;
